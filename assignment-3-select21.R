...
# loading the dataset
df <- read_csv("transactions-2.csv")
...
df$income <- as.factor(df$income1)
df$type <- as.factor(df$type)
df$same_city <- as.factor(df$same_city)
df$same_count <- as.factor(df$same_count)
...
df1$income <- as.factor(df1$income1)
df1$type <- as.factor(df1$type)
df1$same_city <- as.factor(df1$same_city)
df1$same_count <- as.factor(df1$same_count)
# one hot encode categorical data 
...
train <- SMOTE(fraud ~ ., train, perc.over = 100, perc.under=200)
...
tb_model <- train(fraud ~ ., data = train, method = "treebag", trControl = control)
...
df1 <- cbind(df1,actual)
print(df1)
write.csv(df1,"/Curso-ML/Assignment-3/classified-final.csv")
...
ggplot(df2, aes(x=age, fill=I("blue"))) +
...
ggplot(df1, aes(income1)) +
  geom_bar(aes(fill = income1)) + 
  theme_minimal() +
  theme(axis.text.y = element_text(size = 12, angle = 90),
        axis.text.x = element_text(size = 12)) +
  scale_y_continuous(labels = scales::comma) +
...
ggplot(df1, aes(x=seller_sco, fill=I("purple"))) +
  ...
  theme_minimal() +
  ggtitle("Seller score") + 
  theme(legend.position = "none")

# same_count?

ggplot(df1, aes(same_count)) +
  ...
  scale_y_continuous(labels = scales::comma) +
  geom_text(stat = 'count',aes(label =..count.., vjust = -0.5)) +
  ggtitle("Same country Distribution")


ggplot(df1, aes(same_city)) +
  ...
  scale_y_continuous(labels = scales::comma) +
  geom_text(stat = 'count',aes(label =..count.., vjust = -0.5)) +
  ggtitle("Same city Distribution")



ggplot(df2, aes(x=dispersao, fill=I("purple"))) +
  geom_histogram(position="identity", bins = 14) +
  theme_minimal() +
  ggtitle("Dispersion of the value form the average") + 
  theme(legend.position = "none")

ggplot(df2, aes(x=V11, fill=I("purple"))) +
  geom_histogram(position="identity", bins = 14) +
  theme_minimal() +
  ggtitle("Grouped by chance to be a fraud") + 
  theme(legend.position = "none")

ggplot(df1, aes(x=V12, fill=I("purple"))) +
  geom_histogram(position="identity", bins = 14) +
  theme_minimal() +
  ggtitle("Grouped by Node") + 
  theme(legend.position = "none")

ggplot(df2, aes(x=V12.1, fill=I("purple"))) +
  geom_histogram(position="identity", bins = 14) +
  theme_minimal() +
  ggtitle("Grouped by pontos") + 
  theme(legend.position = "none")
